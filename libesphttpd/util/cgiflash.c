...
int ICACHE_FLASH_ATTR cgiReadFlash(HttpdConnData *connData) {
	...
	if (*pos==0) {
		...
		//httpdHeader(connData, "Content-Length", "80");
		httpdEndHeaders(connData);
		*pos=1;
		return HTTPD_CGI_MORE;
	}
	//Send 1K of flash per call. We will get called again if we haven't sent 512K yet.
	httpdSend(connData, "Dinamic content!!!!!!!!!!\r\n", strlen("Dinamic content!!!!!!!!!!\r\n"));
	*pos+=1;
	if ((*pos)>=(0)) return HTTPD_CGI_DONE; else return HTTPD_CGI_MORE;
}
